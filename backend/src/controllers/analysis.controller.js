...
import { geminiJsonRequest, geminiRequest } from '../integrations/gemini/geminiClient.js';
...
async function buildRagInsights({ resume, selectedRole, skillGaps }) {
  const technologiesToLearn = skillGaps.slice(0, 6);
  const ragContext = [
    `Role: ${selectedRole}`,
    `Skills: ${resume.skillsFlat.join(', ') || 'None'}`,
    `Projects: ${(resume.projects || []).map((project) => project.title).join(', ') || 'None'}`,
    `Certifications: ${(resume.certifications || []).join(', ') || 'None'}`,
    `Gaps: ${skillGaps.join(', ') || 'None'}`,
  ].join('\n');
  const strengths = await geminiRequest(
    `List 3-5 concise strengths for this internship profile as JSON array of strings.\n${ragContext}`
  );
  const weaknesses = await geminiRequest(
    `List 3-5 concise weaknesses for this internship profile as JSON array of strings.\n${ragContext}`
  );
  const recommendations = await geminiJsonRequest(
    `Return JSON with keys technologiesToLearn, resumeSuggestions, suggestedProjects, weeklyPlan, monthlyPlan, estimatedCompletionTime, projectNarratives. Keep lists short and role-specific.\n${ragContext}`
  ).catch(() => null);
  ...
  const geminiNarratives = normalizeProjectNarratives(parseJsonPayload(recommendations?.text)?.projectNarratives || []);
  const projectInsights = {
    resumeDerived: resumeProjects,
    ragEnriched: ragProjects,
    geminiNarrative: geminiNarratives,
  };
  const certificationInsights = (resume.certifications || []).map((certification) => ({
    name: certification,
    industryRelevance: 'Relevant to internship readiness',
    knowledgeAreas: ['Foundational knowledge'],
  }));
  const parsedRecommendations = parseJsonPayload(recommendations?.text) || {};
  const recommendationPayload = {
    ...
    resumeSuggestions: [
      ...
      ...(Array.isArray(parsedRecommendations.resumeSuggestions) ? parsedRecommendations.resumeSuggestions : []),
    ],
    suggestedProjects: [
      { title: `${selectedRole} capstone`, reason: 'Demonstrate applied skills' },
      { title: 'Portfolio improvement sprint', reason: 'Show measurable outcomes' },
      ...(Array.isArray(parsedRecommendations.suggestedProjects) ? parsedRecommendations.suggestedProjects : []),
    ],
    weeklyPlan: Array.isArray(parsedRecommendations.weeklyPlan) ? parsedRecommendations.weeklyPlan : [],
    monthlyPlan: Array.isArray(parsedRecommendations.monthlyPlan) ? parsedRecommendations.monthlyPlan : [],
    estimatedCompletionTime: parsedRecommendations.estimatedCompletionTime || '6 months',
  };
  const roadmap = [
    { level: 'Beginner', month: 'Month 1', items: ['Core fundamentals', ...technologiesToLearn.slice(0, 1)] },
  ...
  const strengthsBullets = normalizeInsightList(strengths?.text || strengths);
  const weaknessesBullets = normalizeInsightList(weaknesses?.text || weaknesses);
  return {
    ...
    careerInsights: {
      ...
      secondary_domains: [],
      domain_match_percentage: 75,
      confidence_score: 78,
    },
    ragSummary: { summary_bullets: strengthsBullets.slice(0, 3) },
    contextPreview: resume.projects.slice(0, 3).map((project) => ({ title: project.title, type: 'project', description: project.summary || '' })),
